...
void CAlfabetos::na(const string ida){
    string alfabet;
    cin.ignore();
    it = Malfas.find(ida);
    if(it == Malfas.end()){
...
void CAlfabetos::ba(const string ida){
...
char CAlfabetos::codisus(char m, char c) const{
    if(m == -1) return -1;
    return it->second.codificar_sustitucion(m, c);

}

char CAlfabetos::codisusespe(char m, char c) const{
    if(m == -1) return -1;
    ...
    if(it == Malfas.end()) cout << "error: el alfabeto no existe" << endl;
        ...
        if(it->second.esespecial()){
            for(int i = 0; i < m.size(); ++i){
                ...
                cout << a;
                a = it->second.decodificar_sustitucion_especial(m[i], clave[j]);
                ++j;
                if(j == clave.size()) j = 0;
            }
        }
            ...
            for(int i = 0; i < m.size(); ++i){
                ...
                cout << a;
                a = it->second.decodificar_sustitucion(m[i], clave[j]);
                ++j;
                if(j == clave.size()) j = 0;
            }
        }
